...
fn main() {
    ...
    let fold_fct_task = |acc, x: Result<String, _>| {
        if let Ok(x) = x {
            ...
            if (elf1_low_bound >= elf2_low_bound && elf1_low_bound <= elf2_high_bound)
                || (elf1_high_bound >= elf2_low_bound && elf1_high_bound <= elf2_high_bound)
                || (elf2_low_bound >= elf1_low_bound && elf2_low_bound <= elf1_high_bound)
                || (elf2_high_bound >= elf1_low_bound && elf2_high_bound <= elf1_high_bound)
            {
                return acc + 1;
            }
        }
